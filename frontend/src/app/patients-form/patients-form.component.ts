...
export class PatientsFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.userService.getPatient(logged).subscribe((p: Patient) => {
      ...
      this.patient = p;
      this.pictureLocation = this.pictureLocation + this.patient.slika;
    })
  }
  ...
  changePassword(){
    if(this.newPassword!=null && this.oldPassword!=null && this.confirmPassword!=null){
      if(this.oldPassword!=this.patient.lozinka){
      ...
      if(this.newPassword!=this.confirmPassword){
        ...
        this.userService.updatePassword(this.patient.korIme,this.newPassword).subscribe((p: Patient) => {
          localStorage.clear();
  ...
  updateInfo(){ 
    if(this.newAdress==undefined && this.newUsername==undefined && this.newTelephone==undefined && this.newEmail==undefined && this.newPicture==undefined){
      ...
      this.userService.getPatient(this.newUsername).subscribe((r:Patient)=>{
        if(r==null){
          if(this.newEmail!=undefined){if(!this.regexEmail.test(this.newEmail)){alert("Email nije u dobrom formatu");return;}}  
          this.userService.checkEmail(this.newEmail).subscribe((e:Patient)=>{
            if(e==null){      
              if(this.oldPassword!=this.patient.lozinka){
                alert("Pogresna lozinka");
              }
              else{    
                this.userService.updateInfo(this.patient.korIme,this.newUsername,this.newAdress,this.newTelephone,this.newEmail,this.newPicture).subscribe((p:Patient)=>{
                  if(p){
